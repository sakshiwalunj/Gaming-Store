...
function displayTable() {
    ...
    // html = `
    // <h1 align='center'>Bill recipt</h1>
    // <table class='table table-striped table-bordered'><thead><tr><th scope='col'>Srno</th><th scope='col'>Product Name</th><th scope='col'>Product Price(in Rs)</th></tr></thead><tbody>`;
    // let totalBill = 0;
    // if(products != null && products.length > 0) {
    //     for(var i = 0; i < products.length; i++) {
    //         totalBill += Number.parseFloat(products[i]['price']);
    //         html += "<tr>";
    //         html += `<td> ${i+1}</td>`;
    //         html += `<td> ${products[i]['name']} </td>`;
    //         html += `<td align='center'> ${products[i]['price']} </td>`;
    //         // html += "<td><button class='btn btn-warning' type='button' onclick='deleteCartRow(this)'>Cancel</button></td>";
    //         html += "</tr>";
    //     }
    // }
    // html+="</tbody></table>";
    // let b = `<p>Total Price : Rs.${totalBill}</p>`

    // const bill = window.open('#', '_target');
    // bill.document.write(html + b);
    // bill.print();
}
...
function sendPurchasedProducts() {
    ...
    var products = JSON.parse(localStorage.getItem("products"));
    //const username = document.getElementById('username-main');

    html = `
    <h1 align='left'>Customer Name - ${username}</h1>
    <h3 align='center'>Bill recipt</h1>
    <table class='table table-striped table-bordered'><thead><tr><th scope='col'>Srno</th><th scope='col'>Product Name</th><th scope='col'>Product Price(in Rs)</th></tr></thead><tbody>`;
    let totalBill = 0;
    if(products != null && products.length > 0) {
        for(var i = 0; i < products.length; i++) {
